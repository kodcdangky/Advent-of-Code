...
outcome = {
    str([None, None, ['a', 'a', 'a', 'a'], None, ['b', 'b', 'b', 'b'], None, ['c', 'c', 'c', 'c'], None, ['d', 'd', 'd', 'd'], None, None]): 0
}


def burrow_sort(burrow):
    if outcome.get(str(burrow), 9999999) != 9999999:
        return outcome[str(burrow)]

    outcome[str(burrow)] = 9999999

    # pulling into rooms
    for r in (2, 4, 6, 8):
        if len(burrow[r]) == 4 or any(x != reservation[r] for x in burrow[r]):
            continue
        ...
        for i in reversed(range(0, r)):
            if type(burrow[i]) is str:
                ...
                if burrow[i] != reservation[r]:
                    ...
                    energy = burrow_sort(burrow) + cost[reservation[r]] * (abs(r - i) + (5 - len(burrow[r])))
            ...
            #                   energy + cost[reservation[r]] * (abs(r - i) + 9 - len(burrow[r]) - len(burrow[i])))
    ...
    for r in (2, 4, 6, 8):
        ...
        for i in reversed(range(0, r)):
            if type(burrow[i]) is str:
                ...
                burrow[i] = burrow[r].pop()
                energy = burrow_sort(burrow) + cost[burrow[i]] * (abs(r - i) + (4 - len(burrow[r])))
                burrow[r].append(burrow[i]); burrow[i] = None
...
if __name__ == '__main__':
    # part 2
    burrow0 = [None, None, ['d', 'd', 'd', 'a'], None, ['d', 'b', 'c', 'c'], None, ['a', 'a', 'b', 'b'], None, ['c', 'c', 'a', 'b'], None, None] # my input
    burrow1 = [None, None, ['a', 'd', 'd', 'b'], None, ['d', 'b', 'c', 'c'], None, ['c', 'a', 'b', 'b'], None, ['a', 'c', 'a', 'd'], None, None] # test
