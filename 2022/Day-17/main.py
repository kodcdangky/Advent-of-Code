...
def day_17(turns):
    def spawn_rock(
        ...
        cols[peak].extend(
            [0] * abs(min(0, len(cols[peak]) - height - (FALL_SPACE + 1)))
        )
        for col in cols:
        ...
        rock.extend(
            (((0,) * rock_height) for _ in range(COLS - len(base_rock) - LEFT_MARGIN))
        )
    ...
    def highest_peak(cols: list[list[int]]) -> tuple[int]:
        ...
        for height, all_col in enumerate(zip(*reversed_cols), start=1):
            for indx, cell in enumerate(all_col):
                if cell:
    ...
    for turn in range(turns):
        ...
        # rock falling
        while True:
            direction = wind[wind_indx]
            wind_indx = (wind_indx + 1) % len(wind)
            rock_bound = (
...
# print(f"Part 2: {day_17(1_000_000_000_000)}")
